...
# Remove invalid High_BP values
df = df[df['High_BP'] != 2.0]

# Fill High_Cholesterol using KNN
imputer = KNNImputer(n_neighbors=15)
...
df['Smoking'] = df['Smoking'].apply(lambda x: df['Smoking'].mode()[0] if x not in [0, 1] else x)

# Drop binning if exists
df = df.drop('Age_Binned', axis=1, errors='ignore')
